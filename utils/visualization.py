...
def visualize_training_logs(
    ...
    validation_every = df["val_metric"].isna().sum()
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(validated_epochs["epoch"], validated_epochs["val_metric"], marker="o", label="Val Metric", color="yellow")

    # Mark best model
    if not validated_epochs.empty:
        best_val_idx = validated_epochs["val_metric"].idxmin()
        best_val = validated_epochs.loc[best_val_idx, "val_metric"]

        best_epoch = validated_epochs.loc[best_val_idx, "epoch"]
